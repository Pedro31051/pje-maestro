...
let activeLiveServer: http.Server | null = null;

export function startLiveServer(liveDir: string, port: number = 49160): Promise<http.Server> {
  if (activeLiveServer && activeLiveServer.listening) {
    return Promise.resolve(activeLiveServer);
  }
      ...
      server.on('error', (err: any) => {
        if (err.code === 'EADDRINUSE') {
          if (retries > 0) {
            ...
            console.log(`[Visual Live Server] Port ${port} bound by external process, reusing instance.`);
            resolve({
              close: (cb?: Function) => {
